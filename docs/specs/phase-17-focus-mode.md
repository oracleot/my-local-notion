...
- [ ] **17.1.1** Extend `Page` interface in `src/types/index.ts`
  - Add `doneColumnId?: string | null` field
- [ ] **17.1.2** Add new types in `src/types/index.ts`
...
- [ ] **17.1.3** Update Dexie schema in `src/lib/db.ts`
...
- [ ] **17.1.4** Create `src/lib/focus-helpers.ts`
...
- [ ] **17.2.1** Extend Zustand store in `src/stores/app-store.ts`
...
- [ ] **17.2.2** Create `src/lib/use-focus-timer.ts` hook
...
- [ ] **17.3.1** Add "Mark as done column" UI in kanban column header
...
- [ ] **17.3.2** Update card filtering to use `getDoneColumnId()` helper

### 17.4 — UI Components

- [ ] **17.4.1** Create `src/components/focus/focus-timer-badge.tsx`
...
- [ ] **17.4.2** Create `src/components/focus/task-picker-dialog.tsx`
...
- [ ] **17.4.3** Create `src/components/focus/day-calendar.tsx`
...
- [ ] **17.4.4** Create `src/components/focus/week-calendar.tsx`
...
- [ ] **17.4.5** Create `src/components/focus/focus-view.tsx`
...
- [ ] **17.4.6** Create `src/components/focus/session-complete-dialog.tsx`
...
- [ ] **17.4.7** Create `src/components/focus/focus-settings-dialog.tsx`
...
- [ ] **17.5.1** Add `/focus` route in `src/App.tsx`
  - Route to `FocusView` component
- [ ] **17.5.2** Update `src/components/layout/sidebar.tsx`
...
- [ ] Create kanban board with custom column names → rightmost column is auto-detected as "done"
- [ ] Mark different column as "done" via column menu → cards in that column excluded from Focus
- [ ] Open Focus view → all non-done cards appear grouped by board
- [ ] Drag card to hour slot → time block created, appears in calendar
- [ ] Click "Start" on time block → timer appears in top bar, counts down
- [ ] Timer reaches 0 → audio plays, completion dialog appears
- [ ] Move card via completion dialog → card moves in original kanban board
- [ ] Extend session → timer adds time, dialog closes
- [ ] Navigate away during active session → timer continues in top bar
- [ ] Delete scheduled card → time block removed automatically
- [ ] Toggle Day/Week view → calendar switches correctly
- [ ] Week view: click day → switches to day view for that date
- [ ] Focus settings: change work duration → new sessions use updated duration
- [ ] Disable audio in settings → no sound on session complete
- [ ] `Cmd+Shift+F` → navigates to Focus view
- [ ] Export includes time blocks and settings
- [ ] Import merges time blocks correctly
